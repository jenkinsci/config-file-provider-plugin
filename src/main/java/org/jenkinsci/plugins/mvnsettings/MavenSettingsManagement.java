 ...
 * 
 */
package org.jenkinsci.plugins.mvnsettings;
...
import org.jenkinsci.plugins.mvnsettings.model.Config;
import org.jenkinsci.plugins.mvnsettings.model.ContentType;
 ...
 */
@Extension
public class MavenSettingsManagement extends ManagementLink implements Saveable {
	 ...
	 * 
	 */
	public MavenSettingsManagement() {
		try {
			this.load();
		} catch (IOException e) {
	...
	public HttpResponse doSaveConfig(StaplerRequest req) {
		...
		try {
			System.out.println("> MavenSettingsManagement.doSaveConfig()");
...
//			req.getView(this, "edit.jelly").forward(req, rsp);
			req.getView(this, "edit.groovy").forward(req, rsp);
		} else {
	...
	public void save() throws IOException {
		System.out.println("****-> MavenSettingsManagement.save()");
	...
	protected XmlFile getConfigXml() {
		return new XmlFile(Jenkins.XSTREAM, new File(Jenkins.getInstance().getRootDir(), "mvn-settings-config.xml"));
	}
